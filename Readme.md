...
**sqlite-vec**（ベクトルインデックス）+ **FTS5**（全文検索）のハイブリッド検索。sqlite-vecがなければ全件スキャン、sentence-transformersもなければ **LIKE検索** にフォールバック。

- ベクトル検索: sqlite-vecのvec0仮想テーブルでk近傍探索（384次元, multilingual-e5-small）
- 全文検索: FTS5でテキスト完全一致。ベクトル検索とのスコア統合（FTSボーナス+0.05）
- 馴化フィルタ: 上位結果同士が類似度0.92以上なら後続を減衰（delusionモード）
...
sqlite-vecのvec0仮想テーブル。memoriesテーブルのembeddingをインデックス化し、MATCH一発でk近傍探索を行う。

| カラム | 何 |
|--------|-----|
| memory_id | memories.idと一致 |
| embedding | FLOAT[384]。memoriesテーブルのembedding BLOBと同期 |

記憶の追加・更新・統合時に自動同期される。sqlite-vec未インストール時はテーブルが作られず、旧方式（全件スキャン）にフォールバック。
